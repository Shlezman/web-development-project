    ...
    async sendOrderToBackend() {
        ...
        try {
            ...
            if (!response.ok) {
                ...
                throw new Error(errorData.errors?.[0]?.msg || 'Failed to get orders history');
            }
...
// Initialize and load the order history
const orders = new Orders();
document.addEventListener('DOMContentLoaded', () => {
    orders.loadCartFromBackend();
    document.getElementById('sendOrderToBackend').addEventListener('click', () => orders.sendOrderToBackend());
});
